/* ============================================================
   ...
   Text: Wang Dun passage (Zizhi Tongjian), FangSong Rain.
...
(function () {
  ...
  const glyphs =
    "春正月郭璞複上疏請因皇孫生下赦令帝從之乙卯大赦改元王敦以璞爲記室參軍璞善卜筮知敦必爲亂己預其禍甚憂之大將軍掾穎川陳述卒璞哭之極哀曰嗣祖焉知非福也敦既與朝廷乖離乃羈錄朝士有時望者置己幕府以羊曼及陳國謝鯤爲長史曼祜之兄孫也曼鯤終日酣醉故敦不委以事敦將作亂謂鯤曰劉隗奸邪將危社稷吾欲除君側之惡何如鯤曰隗誠始禍然城狐社鼠敦怒曰君庸才豈達大體出爲豫章太守又留不遣戊辰敦舉兵于武昌上疏罪狀劉隗稱隗佞邪讒賊威福自由妄興事役勞擾士民賦役煩重怨聲盈路臣備位宰輔不可坐視成敗輒進軍致討隗首朝懸諸軍夕退".split(
      "",
    );
  ...
  const fs = 24; // glyph size px
  const colW = 50; // column spacing px (larger -> sparser)
  ...
  function setup() {
    ...
    for (let i = 0; i < cols; i++) {
      streams.push({
        ...
        len: 7 + Math.floor(Math.random() * 13),
        cells: {},
      });
    }
  ...
  function frame(now) {
    ...
    last = now;
    ctx.clearRect(0, 0, W, H);
    ctx.font = fs + "px 'FangSong Rain', serif";
    ...
    for (let i = 0; i < cols; i++) {
      ...
      for (let k = 0; k < s.len; k++) {
        ...
        ctx.fillStyle =
          "rgba(21,36,59," + (0.07 + fade * 0.24).toFixed(3) + ")";
      ...
      if ((s.head - s.len) * fs > H) {
        ...
        s.len = 7 + Math.floor(Math.random() * 13);
        s.cells = {};
      }
    }
